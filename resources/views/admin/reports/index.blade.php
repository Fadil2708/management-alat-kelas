...
@section('title', 'Borrowing Reports')
...
@section('page-title', 'Borrowing Reports')
@section('page-subtitle', 'Generate and export borrowing analytics')
...
<div class="animate-fade-in">
        ...
        <div class="bg-white rounded-xl shadow-lg p-4 border border-gray-100">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xl font-bold text-gray-800">{{ $borrowings->count() }}</p>
                </div>
            </div>
        </div>
        <div class="bg-white rounded-xl shadow-lg p-4 border border-gray-100">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xl font-bold text-green-600">{{ $borrowings->where('status', 'returned')->count() }}</p>
                </div>
            </div>
        </div>
        <div class="bg-white rounded-xl shadow-lg p-4 border border-gray-100">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xl font-bold text-blue-600">{{ $borrowings->where('status', 'approved')->count() }}</p>
                </div>
            </div>
        </div>
        <div class="bg-white rounded-xl shadow-lg p-4 border border-gray-100">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xl font-bold text-red-600">{{ $borrowings->where('status', 'rejected')->count() }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-lg p-6 mb-6 border border-gray-100">
        <form method="GET" action="{{ route('admin.reports.index') }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-4">
            <div>
            ...
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-2">Tool</label>
                <select name="tool_id" class="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all">
            ...
            <div class="flex items-end gap-2">
                <button type="submit" class="flex-1 btn-primary text-white px-4 py-2.5 rounded-xl font-semibold shadow-lg">
                ...
                <a href="{{ route('admin.reports.export', request()->query()) }}"
                   class="flex-1 bg-gradient-to-r from-emerald-500 to-green-500 text-white px-4 py-2.5 rounded-xl font-semibold shadow-lg hover:from-emerald-600 hover:to-green-600 transition-all text-center flex items-center justify-center gap-2">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center gap-2">
                                @if($borrowing->tool->photo)
                                    <img src="{{ Storage::url($borrowing->tool->photo) }}" alt="{{ $borrowing->tool->name }}" class="w-8 h-8 rounded-lg object-cover">
                                @else
                                ...
                                @endif
                                <span class="text-sm font-medium text-gray-900">{{ $borrowing->tool->name }}</span>
                            </div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center gap-2">
                                <div class="w-7 h-7 bg-gradient-to-br from-indigo-500 to-purple-500 rounded-full flex items-center justify-center text-white text-xs font-bold">
                                    {{ strtoupper(substr($borrowing->user->name, 0, 1)) }}
                                </div>
                                <span class="text-sm text-gray-900">{{ $borrowing->user->name }}</span>
                            </div>
                        </td>
                    ...
                    <tr>
                        <td colspan="6" class="px-6 py-16 text-center">
                            <div class="max-w-sm mx-auto">
                                ...
                                <p class="text-gray-500 text-lg font-medium mb-2">No records found</p>
